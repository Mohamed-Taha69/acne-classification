...
def build_resnet(model_name: str, num_classes: int, pretrained: bool = True, binary: bool = False) -> Tuple[nn.Module, int]:
    if binary:
        # Binary classification: GlobalAveragePooling -> Dense(256, ReLU) -> Dropout(0.4) -> Dense(1, Sigmoid)
    ...
    # Multi-class classification (original behavior)
    if model_name == "resnet18":
        backbone = models.resnet18(weights=models.ResNet18_Weights.DEFAULT if pretrained else None)
        in_features = backbone.fc.in_features
